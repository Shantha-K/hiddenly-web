...
//           name: chat.contactName || chat.contactMobile,
//           text: chat.lastMessage || "No messages yet",
//           color: "green",
//           mobile: chat.contactMobile,
...
const ChatPage = () => {
  ...
  const fetchContacts = async () => {
    ...
    try {
      const response = await fetch("http://35.154.10.237:5000/api/getAllUser", {
        method: "GET",
        headers: {
          "Content-Type": "application/json", // JSON is standard, "text/plain" not needed
        },
      });
  ...
  const fetchChatList = async () => {
    ...
    try {
      ...
      const raw = JSON.stringify({
        mobile: userData.user_phone || "7676045226", // fallback if userData is missing
      });
        ...
        const chatMessages = result.chats.map((chat) => ({
          ...
          text:
            typeof chat.lastMessage === "object"
              ? chat.lastMessage.content // ✅ pick content if it's an object
